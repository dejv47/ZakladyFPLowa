...
export function findPlayer(scorers, query) {
  ...
  // Pełne imię/nazwisko ma pierwszeństwo. Nie dopasowujemy długiego zapytania
  // po samym firstName (np. "Morgan Gibbs-White" nie może trafić w Morgan Rogers).
  let hit = scorers.find(p => clean(p.name) === q);
  if (hit) return hit;
  ...
  const wanted = (aliases[q] ?? [q]).map(clean);

  hit = scorers.find(p => {
    const full = clean(p.name || `${p.firstName || ""} ${p.lastName || ""}`);
    const last = clean(p.lastName);
    return wanted.some(a => full === a || last === a);
  });
  if (hit) return hit;

  // Dopiero na końcu ostrożny fallback po pełnej nazwie.
  return scorers.find(p => {
    const full = clean(p.name || `${p.firstName || ""} ${p.lastName || ""}`);
    return wanted.some(a => a.length >= 5 && full.includes(a));
  }) ?? null;
}
